  ...
  const loadConversations = async () => {
    ...
    const convs: Conversation[] = userIds.map(uid => {
      ...
      return {
        ...
        lastMessageTime: new Date(entry.lastMsg.created_at).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }),
      ...
      <div className="px-5 space-y-2">
        {filtered.length === 0 && (
          <div className="text-center py-16 text-muted-foreground text-sm">
            {conversations.length === 0 ? "Aucune conversation. Ajoutez des amis pour commencer !" : "Aucun résultat"}
          </div>
        )}
        {filtered.map((conv, i) => (
          <motion.div
