      ...
      <div className="px-5 pt-12 pb-4 flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <button onClick={markAllAsRead}
            className="flex items-center gap-1 text-xs text-primary font-medium px-3 py-1.5 rounded-lg bg-primary/10 border border-primary/20">
      ...
      <div className="px-5 space-y-2">
        ...
        {notifications.map((notif, i) => (
          <motion.button
            ...
            className={`w-full text-left bg-card border rounded-xl p-4 flex items-start gap-3 transition-all shadow-sm hover:shadow-md ${
              notif.read ? "border-border/60 opacity-70" : "border-primary/30 bg-primary/[0.03]"
            }`}
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className={`text-sm font-medium ${notif.read ? "text-foreground" : "text-foreground font-semibold"}`}>
                  {notif.title}
                </p>
