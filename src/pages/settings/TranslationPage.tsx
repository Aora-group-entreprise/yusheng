...
const TranslationPage = () => {
  ...
  const toggleTranslate = (value: boolean) => {
    ...
    toast.success(value ? "Traduction activée ✓" : "Traduction désactivée");
  };
  ...
  return (
    <div className="min-h-screen gradient-bg pb-20">
      <div className="glass border-b border-border/40 px-4 pt-10 pb-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold font-display text-foreground">🌐 Traduction</h1>
      </div>
      <div className="px-5 pt-4 space-y-4">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-foreground">Traduction automatique</p>
            <p className="text-xs text-muted-foreground">Traduire les messages reçus</p>
          </div>
        ...
        {settings?.auto_translate && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-medium text-foreground">Langue cible</p>
            </div>
            <p className="text-xs text-muted-foreground">
              Les messages seront traduits en <span className="font-semibold text-primary">{settings?.language?.toUpperCase()}</span> automatiquement.
            </p>
            <p className="text-[10px] text-muted-foreground mt-2">
              Powered by Google Translate / DeepL via Lovable Cloud
            </p>
          </motion.div>
        )}
      </div>
