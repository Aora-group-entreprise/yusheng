...
const RoomChatPage = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        const translated = settings?.auto_translate && newMsg.user_id !== user.id
          ? await translateText(newMsg.content)
          : null;
  ...
  const translateText = async (text: string): Promise<string | null> => {
    try {
      const { data, error } = await supabase.functions.invoke("translate", {
        body: { text, targetLang: settings?.translate_target || settings?.language || "fr" },
      });
  ...
  const loadMessages = async () => {
    ...
    const enriched = await Promise.all(data.map(async (msg) => {
      ...
      if (settings?.auto_translate && msg.user_id !== user?.id && !translated) {
        translated = await translateText(msg.content);
      }
    ...
    <div className="min-h-screen gradient-bg flex flex-col">
      ...
      <div className="glass border-b border-border/40 px-4 pt-10 pb-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-primary font-medium">Salon public</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-16 text-muted-foreground text-sm">Soyez le premier à écrire ! ✨</div>
        )}
        {messages.map((msg) => {
            ...
            <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="max-w-[80%]">
                {!isMe && (
                  <div className="flex items-center gap-1.5 mb-1 ml-1">
                    <div className="w-5 h-5 rounded-full bg-muted flex items-center justify-center text-[10px] font-semibold text-primary overflow-hidden">
                      ...
                      ) : (msg.pseudo || "U").charAt(0).toUpperCase()}
                    </div>
                ...
                <div className={`px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                  ...
                  {msg.translated_content && msg.user_id !== user?.id ? (
                    <div>
                      <span>{msg.translated_content}</span>
                      <p className="text-[10px] opacity-60 mt-1 italic">{msg.content}</p>
                    </div>
                  ) : (
